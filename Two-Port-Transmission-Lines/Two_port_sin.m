...
%------------------ 50 Ohm Microstrip transmission line with ABC ----------------------
...
strip_x1 = round(0.5*(Nx-strip_w/dx));
Nw = round(strip_w/dx);
...
feed_x = strip_x1 + round(0.5*strip_w/dx); 		% center of strip
feed_y = strip_y1;
...
while(N_steps>0)  
    ...
    for n=1:N_steps         
        ...
        pulse = sin(2*pi*5e9*time);               % Frequency = 5 Ghz 
        ey(strip_x1:strip_x2,strip_y1:strip_y2,feed_z1) = pulse/dy;  
